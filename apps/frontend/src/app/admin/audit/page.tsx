  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={exportPdf} className="rounded-xl border border-slate-300 px-4 py-2 text-sm text-black hover:bg-slate-50">Export PDF</button>
      </div>

      <div className="mb-4 flex flex-wrap gap-3">
        <input value={actionType} onChange={(e) => setActionType(e.target.value)} placeholder="Action type" className="rounded-xl border border-slate-300 px-3 py-2 text-sm" />
        <input value={entityType} onChange={(e) => setEntityType(e.target.value)} placeholder="Entity type" className="rounded-xl border border-slate-300 px-3 py-2 text-sm" />
        ...
        <button onClick={() => { setPage(1); loadLogs(); }} className="rounded-xl bg-amber-500 px-4 py-2 text-sm text-black hover:bg-amber-400">Search</button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-700">Time</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-700">Action</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-700">Entity</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-700">Actor</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-700">IP</th>
            </tr>
          </thead>
          <tbody>
            {logs.map((log) => (
              <tr key={log.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-2"><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-black">{log.actionType}</span></td>
                ...
                <td className="px-4 py-2 text-xs text-slate-600">{log.ipAddress ?? '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1}
            className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm disabled:opacity-50">Previous</button>
          <span className="px-3 py-1.5 text-sm text-slate-800">Page {page}</span>
          <button onClick={() => setPage(page + 1)} disabled={logs.length < 50}
            className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm disabled:opacity-50">Next</button>
        </div>
      </div>
    </div>
  );
